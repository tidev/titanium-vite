...
interface RequireExpression {
  // The start position of the specifier
  start: number;
  // The end position of the specifier
  end: number;
  // The start position of the statement
  statementStart: number;
  // The end position of the statement
  statementEnd: number;
  // Whether the require is safe, meaning it is not a dynamic require
  safe: boolean;
  // The next require in the list
  next?: RequireExpression;
}

interface RequireInfo extends RequireExpression {
  // The specifier of the require, i.e. the value inside the parentheses
  specifier?: string;
}
...
export function parseRequires(code: string, filename = "@"): RequireInfo[] {
  ...
  const openTokenStack: OpenToken[] = Array.from({ length: 1024 }, () => ({
    token: 0,
    pos: 0,
  }));
  ...
  const getOpenTokenAtDepth = (depth: number) => {
    const openToken = openTokenStack[depth];
    if (!openToken) {
      throw new Error("Invalid token stack index");
    }
    return openToken;
  };
  ...
  const tryParseRequire = () => {
    ...
    if (ch === "(") {
      getOpenTokenAtDepth(openTokenDepth).token = OpenTokenState.RequireParen;
      getOpenTokenAtDepth(openTokenDepth++).pos = pos;
  ...
  const skipCommentAndWhitespace = (br: boolean) => {
    ...
    do {
      ...
      if (ch === "/") {
        ...
        br ? !isBrOrWs(ch.charCodeAt(0)) : !isWsNotBr(ch.charCodeAt(0))
      ) {
        return ch;
      }
    } while (pos++ < end);
  ...
  const skipTemplateString = () => {
    while (pos++ < end) {
      ...
      if (ch === "$" && code.charAt(pos + 1) === "{") {
        pos++;
        getOpenTokenAtDepth(openTokenDepth).token =
  ...
  while (pos++ < end) {
    ...
    switch (ch) {
      ...
      case "c":
        ...
        break;
      case "(": {
        getOpenTokenAtDepth(openTokenDepth).token = OpenTokenState.AnyParen;
        getOpenTokenAtDepth(openTokenDepth++).pos = lastTokenPos;
        break;
      }
      case ")": {
        if (openTokenDepth === 0) {
        ...
        if (
          requireStackDepth > 0 &&
          getOpenTokenAtDepth(openTokenDepth).token ===
            OpenTokenState.RequireParen
        ) {
          const currentRequire = requireStack[requireStackDepth - 1];
          if (!currentRequire) {
            throw new Error(
              `Invalid require stack index ${requireStackDepth - 1}, current stack size ${requireStack.length}`,
            );
          }
      ...
      case "{":
        ...
        getOpenTokenAtDepth(openTokenDepth).token = nextBraceIsClass
        ...
        if (nextCh === "/") {
          ...
          if (
            (isExpressionPunctuator(lastToken.charCodeAt(0)) &&
              ...
              !(lastToken === "-" && prevLastToken === "-")) ||
            (lastToken === ")" &&
              isParenKeyword(getOpenTokenAtDepth(openTokenDepth).pos)) ||
            (lastToken === "}" &&
              (isExpressionTerminator(
                getOpenTokenAtDepth(openTokenDepth).pos,
              ) ||
                getOpenTokenAtDepth(openTokenDepth).token ===
                  OpenTokenState.ClassBrace)) ||
      ...
      case "`": {
        getOpenTokenAtDepth(openTokenDepth).pos = lastTokenPos;
        getOpenTokenAtDepth(openTokenDepth++).token = OpenTokenState.Template;
        skipTemplateString();
        break;
      }
    }
