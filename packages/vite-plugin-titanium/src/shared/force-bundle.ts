 ...
 *
 * This plugin runs with `enforce: "pre"`, intercepts bare specifiers we have
 * NOT explicitly marked as Titanium-provided builtins (via
 * `titanium:node-builtins`, which runs first), resolves them to absolute file
 * paths via Node resolution, and returns them as non-external. Rolldown then
 * sees a resolved file path (not a package name) and bundles the file.
 */
